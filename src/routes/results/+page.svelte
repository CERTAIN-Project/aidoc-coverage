...
<script lang="ts">
  ...
  import { Alert, P, TabItem, Tabs, Tooltip } from "flowbite-svelte";
  ...
  import {
    ...
    type CompletenessFilterStatus,
    type CompletenessFilterType,
  } from "$lib/components/coverage/completeness-helpers";
  ...
  $: filteredResults =
    response?.results.filter(
      (result) => selectedFilter === "all" || result.status === selectedFilter,
    ) ?? [];
  ...
  $: if (
    ...
    (!selectedResult ||
      !filteredResults.find(
        (result) => result.queryId === selectedResult?.queryId,
      ))
  ) {
    selectedResult = filteredResults[0];
  }

  $: completenessEntries = response?.completeness
    ? flattenCompleteness(response.completeness)
    : [];

  $: filteredCompletenessEntries = completenessEntries.filter(
    (entry) =>
      (selectedCompletenessType === "all" ||
        entry.type === selectedCompletenessType) &&
  ...
  $: if (
    ...
    (!selectedCompletenessEntry ||
      !filteredCompletenessEntries.find(
        (entry) => entry.value === selectedCompletenessEntry?.value,
      ))
  ) {
    ...
    if (raw) {
      try {
        ...
        const hasInstanceData =
          !parsed.completeness ||
          parsed.completeness.results.every((result) =>
            result.entries.every((entry) => Array.isArray(entry.instances)),
          );
        ...
        if (hasInstanceData) {
          ...
          loadError =
            "Stored analysis results are outdated. Run a new analysis from the dashboard to see instance names.";
        }
      } catch {
        loadError =
          "Stored analysis results could not be read. Run a new analysis from the dashboard.";
      }
    }
  }

  $: if (browser && response && !hasLoggedQueryErrors) {
    const failedQueries = response.results.filter(
      (result) => result.status === "error",
    );

    if (failedQueries.length) {
      console.groupCollapsed(
        `[Coverage] ${failedQueries.length} query evaluation error(s)`,
      );

      for (const result of failedQueries) {
        console.error(`[${result.queryId}] ${result.title}`, {
          message:
            result.errorMessage ?? "No detailed error message available.",
...
<div class="lg:container page-shell space-y-8">
  <header class="space-y-3">
    <div class="space-y-2">
      ...
      <p class="max-w-3xl text-base text-slate-600 dark:text-slate-300">
        Review the latest competency-query analysis and inspect evidence for
        each query.
      </p>
    </div>
  </header>
          ...
          {#snippet titleSlot()}
            <div class="flex gap-2 items-center">
              <Tooltip
                ...
                trigger="hover"
                triggeredBy="#coverage-info"
                >Coverage measures how fully the instantiated ontology satisfies
                the competency questions and requirements defined by the
                originating reference ontology. Each competency query checks
                whether expected assertions and knowledge graph structures are
                present in the application graph.
              </Tooltip>
              Coverage
              <InfoCircleOutline id="coverage-info" />
            </div>
          {/snippet}
          <div class="space-y-6">
            <ResultsFilterBar
              selected={selectedFilter}
              on:change={(event) => (selectedFilter = event.detail)}
            />

            {#if filteredResults.length}
              <div
                class="grid grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)] items-stretch gap-6"
              >
                ...
                <div
                  ...
                  style="height: {resultsTableHeight}px"
                >
            ...
            {:else}
              <div class="section-card p-4">
                <p class="text-sm text-slate-700 dark:text-slate-200">
                  No results in this filter. Try another filter to inspect
                  competency queries from a different status group.
                </p>
              </div>
            {/if}
          </div>
        ...
        <TabItem title="Completeness">
          {#snippet titleSlot()}
            <div class="flex gap-2 items-center">
              <Tooltip
                class="w-1/2 text-base"
                trigger="hover"
                triggeredBy="#completeness-info"
                >Completeness evaluates schema term usage by checking whether
                the classes, object properties, datatype properties, and named
                individuals defined in the originating reference ontology are
                actually instantiated or referenced in the application graph.
              </Tooltip>
              Completeness
              <InfoCircleOutline
          ...
          <div class="space-y-6">
            <CompletenessFilterBar
              ...
              on:changeType={(event) =>
                (selectedCompletenessType = event.detail)}
              on:changeStatus={(event) =>
                (selectedCompletenessStatus = event.detail)}
            />
            ...
            {#if filteredCompletenessEntries.length}
              ...
              >
                <div
                  class="min-w-0"
                  bind:clientHeight={completenessTableHeight}
                >
                  <CompletenessTable
                    entries={filteredCompletenessEntries}
                    selectedValue={selectedCompletenessEntry?.value ?? ""}
                    on:select={(event) =>
                      (selectedCompletenessEntry = event.detail)}
                  />
                </div>
                <div
                  class="min-w-0 overflow-hidden"
                  style="height: {completenessTableHeight}px"
                >
            ...
            {:else}
              <div class="section-card p-4">
                <p class="text-sm text-slate-700 dark:text-slate-200">
                  No ontology terms in this filter. Try another filter to
                  inspect a different category or status.
                </p>
              </div>
            {/if}
          </div>
  ...
  {:else}
    ...
    <div class="section-card p-4">
      <a
        href="{base}/"
        class="text-sm font-semibold text-blue-700 underline-offset-2 hover:underline dark:text-blue-400"
      >
        Go to dashboard
      </a>
    </div>
